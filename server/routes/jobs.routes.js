...
/** Open positions and the interview stages configured for each one. */
...
const BASE_SELECT =
  "SELECT j.*, u.name AS created_by_name, m.name AS hiring_manager_name, " +
  "(SELECT COUNT(*) FROM candidates c WHERE c.job_id = j.id) AS candidate_count " +
  "FROM jobs j " +
  "LEFT JOIN users u ON u.id = j.created_by " +
  "LEFT JOIN users m ON m.id = j.hiring_manager ";
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const description = v.str(req.body.description, { field: "Job description", max: 5000 });
    ...
    const hiringManager = req.body.hiringManagerId
...
router.patch(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (req.body.description !== undefined) {
      push("description", v.str(req.body.description, { field: "Job description", max: 5000 }));
    }
    ...
    if (req.body.hiringManagerId !== undefined) {
      push(
        "hiring_manager",
        req.body.hiringManagerId ? v.id(req.body.hiringManagerId, { field: "hiring manager" }) : null
      );
    }
